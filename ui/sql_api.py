...
class Bd:
    ...
    def get_all_group(self) -> list:
        return self.cur.execute("SELECT id, name, num_students, teacher FROM groups").fetchall()
    ...
    def get_net_id_group(self, group):
        return self.cur.execute(f"SELECT vk_id, Telegram_id FROM students WHERE groups = {group}").fetchall()
    ...
    def get_teacher_by_id(self, id:int) -> str:
        return self.cur.execute(f"SELECT name FROM teacher WHERE id = {id}").fetchone()[0]

    def get_id_by_teacher(self, name) -> str:
        return self.cur.execute(f'SELECT id FROM teacher WHERE name = "{name}"').fetchone()[0]

    def get_group_by_id(self, id:int) -> list:
        return self.cur.execute(f'SELECT * FROM groups WHERE id = {id}').fetchone()

    def get_all_students_group(self, id:int) -> list:
        return self.cur.execute(f'SELECT id, name FROM students WHERE groups = {id}').fetchall()

    def add_student(self, groups, name, Phone_number, SNILS, Pers_data,  School, Class, Email, Comment, vk_id, Telegram_id, discord_id):
        student = (None, groups, name, Phone_number, SNILS, Pers_data,  School, Class, Email, Comment, vk_id, Telegram_id, discord_id)
        self.cur.execute("INSERT INTO students VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", student)
        self.conn.commit()
    ...
    def update_student_info(self, groups, name, Phone_number, SNILS, Pers_data, School, Class, Email, Comment, vk_id, Telegram_id, discord_id, id=0):
        student = (groups, name, Phone_number, SNILS, Pers_data,  School, Class, Email, Comment, vk_id, Telegram_id, discord_id)
        print(student)
        self.cur.execute(
            f"""UPDATE students
            SET
    ...
    def edit_group(self, id, name, description, num_students, teacher):
        self.cur.execute("UPDATE groups SET name = ?, description = ?, num_students = ?, teacher = ? WHERE id = ?", (name, description, int(num_students), int(teacher), id))
        self.conn.commit()
